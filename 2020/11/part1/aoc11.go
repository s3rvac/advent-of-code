...
func (layout *SeatLayout) getOccupiedSeatCountInNeighborhoodOfCell(i, j int) int {
	count := 0

	if layout.isCellOccupied(i-1, j-1) {
		count += 1
	}
	if layout.isCellOccupied(i-1, j) {
		count += 1
	}
	if layout.isCellOccupied(i-1, j+1) {
		count += 1
	}
	if layout.isCellOccupied(i, j-1) {
		count += 1
	}
	if layout.isCellOccupied(i, j+1) {
		count += 1
...
func (layout *SeatLayout) getNewCellState(i, j int) rune {
	...
	case 'L':
		...
		if layout.getOccupiedSeatCountInNeighborhoodOfCell(i, j) == 0 {
			return '#'
		}
	case '#':
		// If a seat is occupied (#) and four or more seats adjacent to it are
		// also occupied, the seat becomes empty.
		if layout.getOccupiedSeatCountInNeighborhoodOfCell(i, j) >= 4 {
			return 'L'
		}
	}

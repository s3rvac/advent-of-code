...
func parseRulesAndMessagesFromString(s string) (*Rules, Messages, error) {
	...
	// From the assignment:
	//
	//     Completely replace rules 8 and 11 with the following:
	//     8: 42 | 42 8
	//     11: 42 31 | 42 11 31
	if _, exists := rules.ruleMap[8]; exists {
		rule8, _ := parseRuleFromString("8: 42 | 42 8")
		rules.ruleMap[8] = rule8
	}
	if _, exists := rules.ruleMap[11]; exists {
		rule11, _ := parseRuleFromString("11: 42 31 | 42 11 31")
		rules.ruleMap[11] = rule11
	}

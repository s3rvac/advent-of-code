...
def some_bricks_can_fall(bricks):
    for i, brick in enumerate(bricks):
        if new_brick := try_moving_brick_down(brick, i, bricks):
            if new_brick != brick:
                return True
...
def count_bricks_that_can_be_safely_disintegrated(bricks):
    count = 0
    for brick in bricks:
        if not some_bricks_can_fall(bricks.without(brick)):
            count += 1
    return count
...
def run_program(input):
    ...
    return count_bricks_that_can_be_safely_disintegrated(bricks)

...
def get_number_of_rolls_removable_by_forklift(diagram):
    def is_roll_removable(i, j):
        adjacent_rolls = 0
    ...
    while diagram_changed:
        ...
        for i in range(0, len(diagram[0])):
            for j in range(0, len(diagram)):
                if diagram[i][j] == "@":
                    if is_roll_removable(i, j):
                        removable_rols += 1
                        diagram[i][j] = "."
                        diagram_changed = True
    return removable_rols


def run_program(input):
    diagram = parse_input(input)
    return get_number_of_rolls_removable_by_forklift(diagram)

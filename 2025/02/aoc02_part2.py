#
# Advent of Code 2025, day 02, part 2
#

import textwrap
import unittest


def read_input():
    with open("input", encoding="utf-8") as f:
        return f.read()


def parse_input(input):
    return [tuple(map(int, r.split("-"))) for r in input.strip().split(",")]


def gen_invalid_ids(ranges):
    # An ID is invalid if it is made only of some sequence of digits repeated
    # at least twice.
    def is_invalid_id(id):
        s = str(id)
        for n in range(1, len(s) // 2 + 1):
            if can_be_split_into_n_groups(s, n):
                return True
        return False

    for min, max in ranges:
        for id in range(min, max + 1):
            if is_invalid_id(id):
                yield id


def can_be_split_into_n_groups(s, n):
    # Can the string `s` be split into consecutive substrings each of length
    # `n` so that each substring is identical to each other?
    #
    # Examples:
    # - split_into_n_groups("0101", 2) -> yes ("01" and "01")
    # - split_into_n_groups("0111", 2) -> no ("01" differs from "11")
    #
    ref = s[:n]
    for i in range(n, len(s), n):
        if s[i : i + n] != ref:
            return False
    return True


def run_program(input):
    ranges = parse_input(input)
    invalid_ids = gen_invalid_ids(ranges)
    return sum(invalid_ids)


if __name__ == "__main__":
    result = run_program(read_input())
    print(result)


class Tests(unittest.TestCase):
    def test_program_returns_correct_result_for_example_input(self):
        input = textwrap.dedent(
            """
            11-22,95-115,998-1012,1188511880-1188511890,222220-222224, 1698522-1698528,446443-446449,38593856-38593862,565653-565659, 824824821-824824827,2121212118-2121212124
            """
        )

        result = run_program(input)

        self.assertEqual(result, 4174379265)
